...
def error_sd(n, x_max, p, c, cfl, n_period, init_cond, temporal_method):
    # Spatial
    mesh = np.linspace(0, x_max, n + 1)
    method = spatial.SpectralDifferenceMethod(mesh, p, c)
    ...
    # Solving
    sol = temporal_method(y0, t, method.rhs)

    x_exp, y_exp = sd_error.extrapolate_sd(method, init_cond((x - c * t[-1]) % x_max))
    x_com, y_com = sd_error.extrapolate_sd(method, sol[-1])
    ...
    ax1.set_title('Errors at t = {0} :\n'.format(t[-1]) + r'$L_1$ (mean) : {0:0.3E}'.format(np.mean(err))
                  + '\t' + r'$L_2$ : {0:0.3E}'.format(np.sqrt(np.mean(err ** 2)))
                  + '\t' + r'$L_\infty$ (max) : {0:0.3E}'.format(np.max(err)))
    ax1.plot(x, init_cond((x - c * t[-1]) % x_max), label='Exact', lw=3)
    ax1.plot(x_exp, y_exp, label='Expected')
    ax1.plot(x_com, y_com, label='{0} + {1}'.format(temporal_method.__name__, spatial_method.__name__))
    ax1.legend()
...
if __name__ == '__main__':
    solve(n=50, x_max=1, p=4, c=1, cfl=1, n_period=10,
          init_cond=initial_condition.sine(1),
          spatial_method=spatial.SpectralDifferenceMethod, temporal_method=temporal.rk_4)

    error_sd(n=50, x_max=1, p=4, c=1, cfl=1, n_period=10,
             init_cond=initial_condition.rect(1), temporal_method=temporal.rk_4)
