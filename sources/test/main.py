...
def solve(n, x_max, order, c, cfl, n_period, initial_condition, spatial_method, temporal_method):
    # Spatial
    ...
    ax1.plot(x, initial_condition((x - c * t[-1]) % x_max), label='Expected', lw=3)
    ...
    ax2.grid(True)
    ax2.set_xlabel(r'$t$')
    err_1 = [np.mean(abs(y[i] - initial_condition((method.x - c * t[i]) % x_max))) for i in range(len(t))]
...
def compare_spatial_order(n_min, x_max, order_max, c, dt, n_period, initial_condition, spatial_method, temporal_method):
    # Temporal
